...
public class Qlearning_class {

	private long startTime;
	private static Vector<State> vec_States;
	private static Vector QlearningValues = new Vector<>();
	private static double gamma;
	private int iterations;
	private double alpha;
	private double epsilon;

	public Qlearning_class(Vector<State> States_vec, double ga, int it, double al, double ep) {
		...
		Q_Learning(iterations, alpha, epsilon);
	...
	private static void Q_Learning(int iterations, double alpha, double epsilon) {
		...
		QlearningValues.add(LearningOptimalPolicy(Qlearning, vecPolicy));

	}
	private static void RunPolicy(Vector<QLearning> Qlearning, Vector<Policy> vecPolicy) {
		// TODO Auto-generated method stub
		...
		for (int i = 0; i < vec_States.size(); i++) {
			...
			for (int j = 0; j < Qlearning.size(); j++) {
				QLearning sarsa = Qlearning.get(j);
			...
			if (vecPolicy.size() == vec_States.size()) {
				vecPolicy.set(i, new Policy(state.getPosition(), direction));
			} else {
				vecPolicy.add(new Policy(state.getPosition(), direction));
	...
	private static void CreateFileWithData() {
		...
		PrintWriter writer;
		try {
			writer = new PrintWriter("Qlearning_results.txt", "UTF-8");

			for (int i = 0; i < QlearningValues.size(); i++) {
				writer.println("" + QlearningValues.get(i));
			}
