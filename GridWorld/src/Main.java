...
public class Main {
		...
		while ((pos !=WinState) && (pos != LoseState)) {
			...
			int nextpos = pos;
				...
				for (int i = 0; i < directions.size(); i++) {
					...
					boolean overshootSelect = directions.get(i).isOvershoot() == Boolean.getBoolean(String.valueOf(randomOvershoot));--- ver como ultrapassar isto
					if (directionExist) {// check only if the direction exist
						//next step its see if the overshoot is selected or not
	...
	private static void InitializeGrid() {
		...
		e.setNextposition(23);
		e.setOvershoot(false);
		vec_directions.add(e);e = new Directions();
		e.setDirection(UP);
		e.setProbability(prob_next);
		e.setNextposition(1);
		e.setOvershoot(false);
